 ...
 * - 后端命令名通过参数注入，消除 useBackupInstall / useCacheInstall / useCacheCleanup
 *   三个 composable 中的重复实现
 *
...
export interface SudoersOptions {
  /** 检测 sudoers 是否已配置的后端命令名（如 check_sudoers_config） */
  checkCommand: string;
  /** 获取 sudoers 配置命令文本的后端命令名（如 get_sudoers_command） */
  getCommand: string;
}
 ...
 *
 * @param options 注入后端命令名（不同场景对应不同检测/获取命令）
...
export function useSudoers(options: SudoersOptions) {
  ...
  async function checkSudoers() {
    try {
      sudoersAvailable.value = await invoke<boolean>(options.checkCommand);
    } catch (e) {
      console.error(`检查 sudoers 配置失败（${options.checkCommand}）:`, e);
  ...
  async function loadSudoersCommand() {
    try {
      sudoersCommand.value = await invoke<string>(options.getCommand);
    } catch (e) {
      console.error(`加载 sudoers 配置命令失败（${options.getCommand}）:`, e);
    }
  }
