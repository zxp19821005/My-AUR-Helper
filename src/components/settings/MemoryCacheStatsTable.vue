<!--
  MemoryCacheStatsTable.vue - 内存缓存运行状态表格组件

  功能：
  - 展示各缓存域的运行状态（已加载/条目数/时间/磁盘文件）
  - 提供「立即写盘」「清空缓存」操作按钮
  - 接收父组件传入的 stats、loading、onFlush、onClear 回调
...
defineProps<Props>();
const emit = defineEmits<{ flush: []; clear: [] }>();
  ...
  <div class="stats-table-root">
    ...
    <div v-else-if="stats" class="stats-list">
      ...
      <div class="stats-actions">
        <StandardizedButton variant="primary" size="sm" @click="() => $emit('flush')">
          立即写盘
        </StandardizedButton>
        <StandardizedButton variant="danger" size="sm" @click="() => $emit('clear')">
          清空缓存
        </StandardizedButton>
      </div>
    </div>
...
.stats-summary { display: flex; flex-wrap: wrap; gap: 0.5rem; }
.stat-chip {
...
.chip-on { background-color: rgba(76, 175, 125, 0.15); color: var(--success); }
.chip-off { background-color: rgba(255, 152, 0, 0.15); color: var(--warning, #ff9800); }
.stats-table { width: 100%; border-collapse: collapse; font-size: 0.8125rem; }
.stats-table th, .stats-table td {
  text-align: left; padding: 0.5rem 0.625rem; border-bottom: 1px solid var(--border);
  color: var(--text-primary); white-space: nowrap;
}
.stats-table th { color: var(--text-secondary); font-weight: 500; }
.state-dot {
  display: inline-block; width: 8px; height: 8px; border-radius: 50%; margin-right: 0.375rem;
}
.dot-on { background-color: var(--success, #4caf7d); }
.dot-off { background-color: var(--text-muted, #9a9cb8); }
.stats-actions { display: flex; gap: 0.5rem; }
</style>
