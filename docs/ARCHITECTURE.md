...
- Tauri IPC 调用: 按领域拆分到 `src/api/`（software/proxy/license/language/settings/backup/cache/dashboard/sudoers），禁止在 composable/store/component/view 中直接 `import { invoke }`
...
├── router/
│   └── index.ts              # 路由配置
├── api/                      # 前端统一 API 抽象层（集中封装全部 Tauri invoke 调用）
│   ├── software.ts           # 软件包领域（CRUD/AUR 同步/上游检查/URL 校验）
│   ├── proxy.ts              # 代理领域（增删改查/批量测试/文件下载解析）
│   ├── license.ts            # License 领域（增删改查/SPDX 同步）
│   ├── language.ts           # 编程语言领域（查询/upsert/删除）
│   ├── settings.ts           # 设置与日志领域（get/set/list + 日志读取/清空/应用）
│   ├── backup.ts             # 备份领域（列表/扫描/去重/安装/删除/子目录）
│   ├── cache.ts              # 缓存领域（列表/扫描/安装/清理/备份到子目录/备份到已有目录）
│   ├── dashboard.ts          # 仪表盘领域（get_dashboard_stats 统计聚合）
│   └── sudoers.ts            # sudoers 免密配置领域（备份安装/缓存安装/缓存清理三组）
├── views/                    # 页面组件（10 个）
│   ├── Dashboard.vue         # 仪表盘
...
│  │             ▼                                         ││
│  │  ┌──────────────────────────┐                          ││
│  │  │  src/api/ 统一 API 抽象层  │  集中封装 invoke 调用     ││
│  │  │  (9 个领域模块)            │  命令字符串/参数形状集中管理││
│  │  └────────────┬─────────────┘                          ││
│  │               ↕ Tauri IPC (invoke)                     ││
│  └──────────────────────────────────────────────────────┘│
│  ┌──────────────────────────────────────────────────────┐│
...
> **架构说明**：前端所有 Tauri IPC 调用统一经 `src/api/` 抽象层封装（按领域拆分 9 个模块），
> composable/store/component/view 不再直接 `import { invoke }`。
