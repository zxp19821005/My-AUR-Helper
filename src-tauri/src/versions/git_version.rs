...
fn re_git_describe() -> &'static Regex {
    ...
    RE.get_or_init(|| {
        Regex::new(r"\.r\d+\.g[a-f0-9]+$").expect("静态正则表达式编译失败: git_describe")
    })
}
...
fn re_r_format_capture() -> &'static Regex {
    static RE: OnceLock<Regex> = OnceLock::new();
    RE.get_or_init(|| {
        Regex::new(r"^r(\d+)\.[a-f0-9]+$").expect("静态正则表达式编译失败: r_format_capture")
    })
}
...
fn re_r_format_match() -> &'static Regex {
    static RE: OnceLock<Regex> = OnceLock::new();
    RE.get_or_init(|| {
        Regex::new(r"^r\d+\.[a-f0-9]{7,}$").expect("静态正则表达式编译失败: r_format_match")
    })
}
