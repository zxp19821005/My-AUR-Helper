...
pub fn compare_versions(aur_version: &str, upstream_version: &str) -> VersionComparison {
    ...
    debug!("[版本标准化]     normalized_version: {}", aur.normalized_version);
    
    let upstream = UpstreamVersion::parse(upstream_version);
    debug!("[版本标准化]   上游版本解析结果:");
    debug!("[版本标准化]     raw: {}", upstream.raw);
    debug!("[版本标准化]     normalized_version: {}", upstream.normalized_version);
    
    debug!("[版本比较] 标准化后比较: AUR={} vs 上游={}", aur.normalized_version, upstream.normalized_version);
