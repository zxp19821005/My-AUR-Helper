...
impl Database {
    ...
    /// @param cs - 缓存软件信息
    /// @returns 新插入记录的 ID
    pub fn insert_cache_software(&self, cs: &CacheSoftware) -> Result<i64> {
        self.conn.execute(
    ...
    /// @returns 该包对应的所有缓存记录
    pub fn get_cache_software_by_pkg(&self, software_id: i64) -> Result<Vec<CacheSoftware>> {
    ...
    /// @returns 所有缓存记录列表
    pub fn get_all_cache_software(&self) -> Result<Vec<CacheSoftware>> {
        let mut stmt = self.conn.prepare(
    ...
    /// 删除缓存记录
    /// @param id - 缓存记录 ID
    pub fn delete_cache_software(&self, id: i64) -> Result<()> {
        self.conn.execute(
